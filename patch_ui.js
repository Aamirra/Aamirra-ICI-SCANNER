...
if (!html.includes(oldIntervalBlock)) {
  console.error('FAILED part 1: interval block not found');
} else {
  html = html.replace(oldIntervalBlock, newIntervalBlock);
  console.log('OK part 1: H/M/L buttons added');
}

// 2. Long/Short toggle + S01-S05 row, right above the search bar
const oldSearch = `    <div class="search-container"><input type="text" class="search-bar" placeholder="Search pairs..." oninput="filterPairs(this.value)"></div>`;
...
if (!html.includes(oldSearch)) {
  console.error('FAILED part 2: search bar block not found');
} else {
  html = html.replace(oldSearch, newSearch);
  console.log('OK part 2: Long/Short + S01-S05 bar added');
}
...
if (!html.includes(oldCss)) {
  console.error('FAILED part 3: CSS anchor not found');
} else {
  html = html.replace(oldCss, newCss);
  console.log('OK part 3: CSS added');
}
...
if (!html.includes(oldJsAnchor)) {
  console.error('FAILED part 4: JS anchor not found');
} else {
  html = html.replace(oldJsAnchor, newJs);
  console.log('OK part 4: JS added');
}
