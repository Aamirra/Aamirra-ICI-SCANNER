...
const CRYPTO_SYMBOLS_NO_WEEKEND_FILTER = new Set(['BTCUSD','ETHUSD','LTCUSD','BCHUSD','FXSUSD','LQTYUSD','MASKUSD']);
function isWeekendGap(unixSeconds) {
...
Always put the action block FIRST, then your reply.`;
    ...
    if (safePath === '/api/candles') {
        ...
        const rawMap = {
            '1h': scannerModule.RAW_1H,
            '4h': scannerModule.RAW_4H,
            '15m': scannerModule.RAW_15M,
            '1day': scannerModule.RAW_DAILY,
            '1week': scannerModule.RAW_WEEKLY
        }[interval];

        if (!rawMap) {
            res.writeHead(400, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ error: `Unsupported interval: ${interval}. Use 1h, 4h, 15m, 1day, or 1week.` }));
            return;
        }

        const d = rawMap[symbol];
        if (!d || !Array.isArray(d.times) || !Array.isArray(d.closes)) {
            res.writeHead(404, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ error: `No cached data for ${symbol} (${interval})` }));
            return;
        }

        const len = d.times.length;
        const candles = [];
        for (let i = 0; i < len; i++) {
            const t = d.times[i];
            const o = d.opens ? d.opens[i] : null;
            const h = d.highs ? d.highs[i] : null;
            const l = d.lows ? d.lows[i] : null;
            const c = d.closes[i];
        ...
        const finalCandles = CRYPTO_SYMBOLS_NO_WEEKEND_FILTER.has(symbol) ? candles : candles.filter(c => !isWeekendGap(c.time));

        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ symbol, interval, candles: finalCandles }));
        return;
    }

    if (safePath === '/api/crypto-chart') {
        (async () => {
            try {
                const urlParams = new URL(req.url, `http://${req.headers.host}`).searchParams;
                const symbol = urlParams.get('symbol');
    ...
    if (safePath === '/api/stock-chart') {
        (async () => {
            try {
                const urlParams = new URL(req.url, `http://${req.headers.host}`).searchParams;
                const symbol = urlParams.get('symbol');
                const market = urlParams.get('market') || 'exness';
                const interval = urlParams.get('interval') || '1h';
                const totalWanted = Math.min(parseInt(urlParams.get('limit')) || 15000, 15000);
    ...
    if (req.method === 'GET' && safePath.startsWith('/api/chat-msg/media/')) {
    ...
    if (req.method === 'POST' && safePath === '/api/chat-msg/send') {
        let body = '';
        req.on('data', chunk => { body += chunk.toString(); });
        req.on('end', async () => {
            try {
    ...
    if (req.method === 'POST' && safePath === '/api/chat-msg/clear') {
    ...
    if (req.method === 'GET' && safePath.startsWith('/api/chat-msg/avatar/')) {
        (async () => {
            try {
